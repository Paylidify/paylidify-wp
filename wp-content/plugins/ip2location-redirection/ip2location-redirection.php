...
class IP2LocationRedirection
{
	...
	public function download_database()
	{
		try {
			...
			if (is_file(IP2LOCATION_REDIRECTION_ROOT . 'database.zip')) {
			...
			if (filesize(IP2LOCATION_REDIRECTION_ROOT . 'database.zip') < 51200) {
				die(file_get_contents(IP2LOCATION_REDIRECTION_ROOT . 'database.zip'));
			}
			...
			while ($entries = zip_read($zip)) {
				...
				if (!is_file(IP2LOCATION_REDIRECTION_ROOT . $file_name)) {
					die('ERROR');
				}
						...
						<div id="bin_database">
							...
							<table class="form-table">
								<tr>
									...
									<td>
										<div>' . ((!is_file(IP2LOCATION_REDIRECTION_ROOT . get_option('ip2location_redirection_database'))) ? '<span class="dashicons dashicons-warning" title="Database file not found."></span>' : '') . get_option('ip2location_redirection_database') . '
									</td>
								</tr>
								<tr>
									<th scope="row">
	...
	public function redirect()
	{
		...
		if (isset($_SESSION['ip2location_redirection_redirected'])) {
			...
			$this->write_debug_log('Redirected. Not checking for redirection anymore.', true);

			return;
		}

		if (!get_option('ip2location_redirection_enabled')) {
			$this->write_debug_log('IP2Location Redirection module is disabled. Redirection aborted.', true);

			return;
		}

		if (is_admin() || current_user_can('administrator')) {
			$this->write_debug_log('Logged in as administrator. Redirection aborted.', true);

			return;
		}

		if (get_option('ip2location_redirection_skip_bots') && $this->is_bot()) {
			$this->write_debug_log('Web crawler detected. Redirection aborted.', true);
		...
		if (get_option('ip2location_redirection_noredirect_enabled')) {
			if (isset($_GET['noredirect']) && trim($_GET['noredirect']) == 'true') {
				$this->write_debug_log('[noredirect] parameter is found. Redirection aborted.', true);
		...
		if (get_option('ip2location_redirection_first_redirect')) {
			if (isset($_COOKIE['ip2location_redirection_first_visit'])) {
				$this->write_debug_log('This is not first visit. Redirection aborted.', true);
		...
		if (($data = json_decode(get_option('ip2location_redirection_rules'))) !== null) {
			...
			if (empty($result['country_code'])) {
				$this->write_debug_log('Unable to identify visitor country. Redirection aborted.', true);
				return;
			}
			...
			foreach ($data as $values) {
				...
				if ($this->is_country_match($result['country_code'], $country_codes)) {
					$this->write_debug_log('Country [' . $result['country_code'] . '] is listed in [' . implode(', ', $country_codes) . '] for redirection.');
					...
					if ($page_from == 'any' || ($page_from == 'home' && $this->is_home()) || ($page_from == 'url' && trim($this->get_current_url(), '/') == trim($url_from, '/'))) {
						...
						if ($id == get_the_ID()) {
							$this->write_debug_log('Redirection aborted.');
				...
				$this->write_debug_log('No redirection rule is matched.', true);
			}
		}
	}
	...
	public function write_debug_log($message, $new_line = false)
	{
	...
	{
		$this->write_debug_log('Redirected To: ' . $url, true);
